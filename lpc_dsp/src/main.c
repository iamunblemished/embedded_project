...
/* USB/Simulink (Console Input) */
#define SIMULINK_UART DT_CHOSEN(zephyr_shell_uart)

/* ESP32 Telemetry (Verified P18 Header Output) */
...
void uart_cb(const struct device *dev, void *user_data)
{
    ...
    if (uart_irq_rx_ready(dev)) {
        if (!data_ready) { 
            ...
            /* When the bucket is full, signal the main thread and go deaf */
...
int main(void)
{
    ...
    while (1) { 
        if (data_ready) {
            ...
            /* 1. Remove DC Offset and Apply Window to real Simulink data */
            for(int i = 0; i < PAYLOAD_SIZE; i++) {
